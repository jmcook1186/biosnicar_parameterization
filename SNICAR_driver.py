...
inputs.solzen   = 35      # if DIRECT give solar zenith angle between 0 and 89 degrees (from 0 = nadir, 90 = horizon)
...
inputs.rho_layers = [650,650] # density of each layer (unit = kg m-3) 
inputs.nbr_wvl=480 
...
inputs.grain_rds = [2000,2000] # effective grain radius of snow/bubbly ice (becomes bubble rds when layer_type==1)
...
inputs.mss_cnc_glacier_algae = [3000,0]   
...
plt.plot(wvl, albedo)
plt.ylabel('ALBEDO'), plt.xlabel('WAVELENGTH (microns)'), plt.xlim(0.2,1.6),
