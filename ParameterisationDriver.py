...
from ParameterisationFuncs import generate_snicar_dataset_single_layer, save_model,\
    regression_single_layer, evaluate_model_performance, test_model_single_layer
...
save_model(modelBBA,str(savepath+'parameterisation_model_BBA_single_layer.pkl'),savepath)
save_model(modelABS,str(savepath+'parameterisation_model_ABS_single_layer.pkl'),savepath)
...
plt.ylim(0.2,0.7),plt.xlim(0.2,0.7),plt.ylabel('parameterisation BBA'),plt.xlabel('snicar BBA')
...
XBBA = test_results_BBA.snicar_BBA
yBBA = test_results_BBA.param_BBA
